...
def sampled_pairwise_distance(new_point_cloud, point_cloud):
    ...
    # og_batch_size_2 = point_cloud.get_shape().as_list()[0]
    # point_cloud = tf.squeeze(point_cloud)
    # new_point_cloud = tf.squeeze(new_point_cloud)
    # if og_batch_size == 1:
    #     point_cloud = tf.expand_dims(point_cloud, 0)
    # if og_batch_size_2 == 1:
    #     new_point_cloud = tf.expand_dims(new_point_cloud, 0)

    point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 2, 1])
    point_cloud_inner = tf.matmul(new_point_cloud, point_cloud_transpose)
    point_cloud_inner = -2 * point_cloud_inner
    point_cloud_square_1 = tf.reduce_sum(tf.square(new_point_cloud), axis=-1, keep_dims=True)
    point_cloud_square_2 = tf.reduce_sum(tf.square(point_cloud), axis=-1, keep_dims=True)
    point_cloud_square_tranpose = tf.transpose(point_cloud_square_2, perm=[0, 2, 1])
    return point_cloud_square_1 + point_cloud_inner + point_cloud_square_tranpose

def get_sampled_edge_feature(point_cloud, old_point_cloud, nn_idx, k=20):
    ...
    #     point_cloud = tf.expand_dims(point_cloud, 0)
    #
    # og_batch_size_2 = old_point_cloud.get_shape().as_list()[0]
    # old_point_cloud = tf.squeeze(old_point_cloud)
    # if og_batch_size_2 == 1:
    #     old_point_cloud = tf.expand_dims(old_point_cloud, 0)

    point_cloud_central = point_cloud

    point_cloud_shape = old_point_cloud.get_shape()
    ...
    point_cloud_flat = tf.reshape(old_point_cloud, [-1, num_dims])

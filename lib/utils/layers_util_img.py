...
def pointnet_sa_module_msg(xyz, points, radius_list, nsample_list, 
                           ...
                           debugging=False,
                           epsilon=1e-5, point_img_feats=None):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for fps_sample_range, fps_method, npoint in zip(fps_sample_range_list, fps_method_list, npoint_list):
            ...
            point_img_feats_ = tf.slice(point_img_feats, [0, last_fps_end_index, 0], [-1, fps_sample_range, -1])

            tmp_points = tf.concat([tmp_points, point_img_feats_], axis=-1)
        ...
        new_point_img_feats = gather_point(point_img_feats, fps_idx)
    ...
    return new_xyz, new_points_concat, fps_idx, new_point_img_feats

...
def tf_gather_object_pc(point_cloud, mask, npoints=256):
...
def pointnet_fps_method(xyz, points, fps_sample_range_list, fps_method_list, npoint_list, scope, vote_ctr=None):
    bs = xyz.get_shape().as_list()[0]
    with tf.variable_scope(scope) as sc:
        cur_fps_idx_list = []
        last_fps_end_index = 0
        ...
        new_points = gather_point(points, fps_idx)

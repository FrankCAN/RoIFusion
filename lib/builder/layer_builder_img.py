...
class LayerBuilder:
    ...
    def build_layer(self, xyz_list, feature_list, fps_idx_list, bn_decay, output_dict, point_img_feats_list):
        """
        Build layers
        """
        xyz_input = []
        ...
        point_img_feats_input = []
        for feature_index in self.feature_index:
            point_img_feats_input.append(point_img_feats_list[feature_index])
        ...
        if self.layer_type == 'SA_Layer':
            new_xyz, new_points, new_fps_idx, new_point_img_feats = pointnet_sa_module_msg(
                ...
                self.dilated_group, vote_ctr, self.aggregation_channel, point_img_feats=point_img_feats_input[0])
            xyz_list.append(new_xyz)
            feature_list.append(new_points)
            fps_idx_list.append(new_fps_idx)
            point_img_feats_list.append(new_point_img_feats)
            ...
            update_feature = tf.concat([feature_input[0], point_img_feats_input[0]], axis=-1)
            new_xyz, new_points, ctr_offsets = vote_layer(xyz_input[0], update_feature, self.mlp_list, self.is_training, bn_decay, self.bn, self.scope)
            # new_xyz, new_points, ctr_offsets = vote_layer(xyz_input[0], feature_input[0], self.mlp_list,
            #                                               self.is_training, bn_decay, self.bn, self.scope)
        ...
        return xyz_list, feature_list, fps_idx_list, point_img_feats_list

...
class DataAugmentor:
    ...
    def kitti_forward(self, points, sem_labels, sem_dists, label_boxes_3d, label_classes, plane, pipename, img_input, img_full_seg):
        """
        ...
        if self.aug_flip and np.random.rand() >= self.aug_flip_prob:
            img_input = kitti_aug.flip_image(img_input)
            img_full_seg = kitti_aug.flip_image(img_full_seg)
        ...
        return points, sem_labels, sem_dists, label_boxes_3d, label_classes, img_input, img_full_seg

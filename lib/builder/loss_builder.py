...
class LossBuilder:
    ...
    def cls_loss(self, index, label_dict, pred_dict):
        ...
        # pred_point_seg_cls = model_util.focal_loss_producer(pred_dict[maps_dict.PRED_POINT_SEG][index],
        #                                       tf.one_hot(label_dict[maps_dict.PL_LABEL_SEMSEGS][index], 2, axis=-1))
        # pred_point_seg_cls = tf.reduce_mean(pred_point_seg_cls, axis=-1)
